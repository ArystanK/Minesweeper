...
import kotlinx.coroutines.flow.flow
import kz.arctan.minesweaper.auth.data.SuchEmailDoesNotExistsException
...
class LoginUseCase @Inject constructor(private val authRepository: AuthRepository) {
    operator fun invoke(email: String, password: String): Flow<ActionResult> = flow {
        ...
        } catch (e: SuchEmailDoesNotExistsException) {
            emit(ActionResult.Error("This email is not present in database"))
        }
    }
}